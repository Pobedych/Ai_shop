...
@router.message(F.text.in_({"💳 Top up your balance now", "💳 Пополнить баланс", "💳 立即充值余额"}))
...
async def money(message: Message, i18n: I18nContext, state: FSMContext):
    async with SessionLocal() as session:
        ...
        try:
            ...
            await message.answer("Введите корректную сумму, без точек, без запятых, только целое число")
            return
...
async def open_language_settings(cq: CallbackQuery, i18n: I18nContext):
    ...
    prompt = prompt_by_locale.get(i18n.locale, "Choose language:")
...
async def set_language(cq: CallbackQuery, i18n: I18nContext):
    ...
    notice = notice_by_locale[locale]
    prompt = prompt_by_locale[locale]
